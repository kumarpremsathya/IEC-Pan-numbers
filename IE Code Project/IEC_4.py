...
def process_captcha_icegate_part3(browser):
    ...
    # Get the CAPTCHA image element
    captcha_image_element = browser.find_element(By.XPATH, '//*[@id="capimg"]')

    # Save the CAPTCHA image locally
    with open('captcha.png', 'wb') as file:
        file.write(captcha_image_element.screenshot_as_png)

    # Preprocess the CAPTCHA image
    captcha_image = cv2.imread('captcha.png')
    captcha_image = cv2.resize(captcha_image, None, fx=12, fy=12, interpolation=cv2.INTER_CUBIC)
    gray_image = cv2.cvtColor(captcha_image, cv2.COLOR_BGR2GRAY)
    ...
    # # Enhance image quality using median blur
    # captcha_image = cv2.medianBlur(captcha_image, 3)
    
    # # Invert the image
    # captcha_image = cv2.bitwise_not(captcha_image)
    
    # # Perform OCR using pytesseract with custom configuration
    # captcha_text = pytesseract.image_to_string(Image.fromarray(captcha_image), config='--psm 8 --oem 3 -c tessedit_char_whitelist=ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789')
...
def process_captcha_dgft_part1(browser):
    try:
        ...
        # captcha_text = ''.join(result).strip()  # Strip any leading/trailing whitespace
        # print("captcha_text", captcha_text)
        ...
        return captcha_text.strip()
                ...
                return None  # Return None if captcha processing fails
        ...
        if table:
            ...
            for row in rows:
                ...
                if cells:
                    ...
                    dict_final = dict(zip(headers, row_details))
                    print("dict_final===", dict_final)
                    rcmc_branch_details.append(dict_final)
           
            print("rcmc_branch_details", rcmc_branch_details)
